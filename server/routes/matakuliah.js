...
router.get("/dashboard/summary", (req, res) => {
  ...
  const ringkasanMk = mkRows.map((row) => {
    ...
    const progress = jumlahMhs > 0 ? Math.round((jumlahTerhitung / jumlahMhs) * 100) : 0;
    const status =
      jumlahMhs === 0
        ? "BELUM_ADA_MAHASISWA"
        : jumlahTerhitung === 0
    ...
    return {
      ...
      rata_nilai_akhir:
        row.rata_nilai_akhir == null ? null : Math.round(Number(row.rata_nilai_akhir) * 100) / 100,
  ...
  const mkBerisiko = [...ringkasanMk]
    .filter((mk) => mk.jumlah_rendah > 0 || (mk.rata_nilai_akhir != null && mk.rata_nilai_akhir < 70))
    .sort((a, b) => (b.jumlah_rendah - a.jumlah_rendah) || ((a.rata_nilai_akhir ?? 100) - (b.rata_nilai_akhir ?? 100)))
    .slice(0, 5);
  ...
  return res.json({
    kpi: {
      ...
      rata_nilai_akhir:
        base.rata_nilai_akhir == null ? null : Math.round(Number(base.rata_nilai_akhir) * 100) / 100,
    },
    distribusi_huruf: {
