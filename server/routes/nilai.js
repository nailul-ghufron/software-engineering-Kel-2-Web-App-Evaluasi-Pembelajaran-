  ...
  try {
    const upsert = db.transaction(() => {
      for (const it of items) {
        ...
        const mhs = db
          .prepare("SELECT id FROM mahasiswa WHERE id = ? AND mata_kuliah_id = ?")
          .get(mid, mkId);
        if (!mhs) {
        ...
        if (tugas.length !== mk.jumlah_tugas) {
          throw new Error(`Setiap mahasiswa harus memiliki ${mk.jumlah_tugas} nilai tugas`);
        }
        tugas = tugas.map((v, i) => {
        ...
        const existing = db.prepare("SELECT id FROM nilai WHERE mahasiswa_id = ?").get(mid);
        if (existing) {
          db.prepare(
    ...
    return res.status(400).json({ error: e.message || "Gagal menyimpan nilai" });
  }
}
...
export function hitungSemuaHandler(req, res) {
  ...
  const update = db.transaction(() => {
    for (const { id: mid } of mahasiswa) {
      let row = db.prepare(`SELECT * FROM nilai WHERE mahasiswa_id = ?`).get(mid);
      if (!row) {
        const empty = JSON.stringify(Array.from({ length: mk.jumlah_tugas }, () => null));
        db.prepare(
          `INSERT INTO nilai (mahasiswa_id, nilai_tugas_json, nilai_uts, nilai_uas) VALUES (?, ?, ?, ?)`
      ...
      const normalized = tugasArr.map((v) => (v === null || v === "" ? 0 : Number(v)));
